...
> **Implementation stack (this repo):** **Zig + Bun + QuickJS** — **Node.js is not part of our implementation** (**`docs/ARCHITECTURE.md`**). Parity work compares **behavior** to upstream’s Node-based CLI, not by embedding Node in **opencliz**.
...
| **L6 plugins** | TS plugins / JS / hooks vs upstream | Done: QuickJS `js_init` + per-command `script` + `opencli.args`/`version`/`log`; optional **`OPENCLI_ENABLE_BUN_SUBPROCESS`** for `ts_legacy` (**no Node** — **`RUNTIME_MODEL.md`**); can still narrow gaps vs full upstream TS API |
...
- [x] `cli-manifest.json` `type: ts`: register **`source=ts_legacy`**; default JSON stub (`status: ts_adapter_not_supported` + `message` + optional `modulePath`). **Optional**: **`OPENCLI_ENABLE_BUN_SUBPROCESS=1`** runs a **Bun** subprocess (**`docs/PLUGIN_QUICKJS.md`**, `runner.zig`); **no** embedded JS runtime and **no Node** in opencliz.
...
- [x] Optional: minimal Zig CI — **`.github/workflows/zig-ci.yml`** (ubuntu/macos, `zig build test`, `ZIG_GLOBAL_CACHE_DIR` + actions/cache). **Chrome subset** may be cross-checked with upstream **`smoke-test`** (Node/Bun tooling on maintainer machines) and **`CDP_SCENARIO_MATRIX.md`**.
...
| **L6** | Full Node API (upstream) | QuickJS subset + optional **Bun** subprocess for `ts_legacy` (timeouts, batch **52**+); **`opencli.http`** bridge + allowlist — **no Node in opencliz** |
...
| **H.3** | ~1–2 months | L3/L6 P2: Chrome CI subset, QuickJS HTTP bridge, **Bun** subprocess hardening — **batch 52 closed** |
