...
### `opencliz antigravity status`
Check the Chromium CDP connection. Returns the current window title and active internal URL.

### `opencliz antigravity send <message>`
Send a text prompt to the AI. Automatically locates the Lexical editor input box, types the prompt securely, and hits Enter.

### `opencliz antigravity read`
Scrape the entire current conversation history block as pure text.

### `opencliz antigravity new`
Click the "New Conversation" button to instantly clear the UI state and start fresh.

### `opencliz antigravity dump`
Dump the current DOM and snapshot artifacts to `/tmp` for reverse-engineering and selector debugging.

### `opencliz antigravity extract-code`
Extract any multi-line code blocks from the current conversation view. Ideal for automated script extraction (e.g. `opencliz antigravity extract-code > script.sh`).

### `opencliz antigravity model <name>`
Quickly target and switch the active LLM engine. Example: `opencliz antigravity model claude` or `opencliz antigravity model gemini`.

### `opencliz antigravity watch`
